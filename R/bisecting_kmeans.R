...
my_kmeans = function(iter.max = 1E3,
                     nstart = 50,
                     ...)
{
  set.seed(1)
...
{
  hs = sapply(l_r_h, function(v)
    v$h)
  all_names = sapply(l_r_h, function(v)
    paste0(collapse = '_', sort(c(v$l, v$r))))
  r_names = sapply(l_r_h, function(v)
    paste0(collapse = '_', sort(c(v$r))))
  
  sizes = sapply(l_r_h, function(v)
    length(v$l) + length(v$r)) ##
...
bisecting_kmeans <- function(data)
{
  ...
  get_sub_tree <- function(indices)
  {
    ...
    l_r_h <<- c(l_r_h, list(list(
      l = l_indices, r = r_indices, h = h
    )))
  ...
  for (i in 1:length(l_r_h)) {
    name = r_hs[i]
    names(name) = paste0(collapse = '_', sort(c(l_r_h[[i]]$l, l_r_h[[i]]$r)))
    l_r_h[[i]]$name = name
  }
  pos_names = sapply(l_r_h, function(v)
    v$name)
  neg_names = -(1:length(indices))
  names(neg_names) = 1:length(indices)
  all_names = c(pos_names, neg_names)
  for (i in 1:length(l_r_h)) {
    l_r_h[[i]]$l_name = unname(all_names[paste0(l_r_h[[i]]$l, collapse = '_')])
    l_r_h[[i]]$r_name = unname(all_names[paste0(l_r_h[[i]]$r, collapse = '_')])
  }
  
  merge_height = data.frame(
    l = sapply(l_r_h, function(v)
      v$l_name),
    r = sapply(l_r_h, function(v)
      v$r_name),
    h = hs
  )
