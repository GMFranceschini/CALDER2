...
CALDER = function(contact_tab_dump = NULL,
                  ...
                  )
{
  ...
  if (n_chrs > 1)
    ## when computing for multiple chrs, contact_tab_dump or contact_file_dump should be a list with elements matching to that of chrs
  {
    if (!is.null(contact_tab_dump))
    {
      if (class(contact_tab_dump) != 'list' |
          length(contact_tab_dump) != n_chrs)
        stop(
          'contact_tab_dump should be a list of contact table with its length equal to the length of chrs you specified\n'
        )
    ...
    {
      if (class(contact_file_dump) != 'list' |
          length(contact_file_dump) != n_chrs)
        stop(
          'contact_file_dump should be a list of contact table with its length equal to the length of chrs you specified\n'
        )
  ...
  if (bin_size == 5E3)
    bin_sizes = c(5E3, 10E3, 50E3, 100E3)
  if (bin_size == 10E3)
  ...
  if (single_binsize_only)
    bin_sizes = bin_size ## do not try multiple bin_sizes
  
  ## choose the already computed good compartment as reference to decide correctly the A/B compartment direction
  if (genome != 'others')
    ref_compartment_file = system.file("extdata",
                                       sprintf('%s_all_sub_compartments.bed', genome),
                                       package = 'CALDER')
  if (genome == 'others')
  ...
  if (is.null(ref_compartment_file) &
      is.null(feature_track))
    stop(
      'Should either specify genome in one of hg19, hg38, mm9, mm10, or provide a feature_track\n'
    )
  ...
  for (bin_size2look in bin_sizes)
  {
    ...
    save_dir_binsize = file.path(save_dir,
                                 ...
                                 bin_size_kb)
    dir.create(save_dir_binsize,
               recursive = TRUE,
               showWarnings = FALSE)
  }
  
  para_tab = data.table::data.table(expand.grid(bin_sizes, chrs))
  for (i in 1:ncol(para_tab))
    para_tab[[i]] = as.vector(para_tab[[i]])
  ...
  silent_out =  foreach::foreach(i = 1:nrow(para_tab), .export = c("predefined_domains")) %dopar%
    {
      ...
      CALDER_CD_hierarchy_v2(
        ...
        contact_file_dump = contact_file_dump[[chr]],
        contact_file_hic = contact_file_hic,
        chr = chr,
        bin_size_input = bin_size,
        bin_size2look = bin_size2look,
        save_dir = save_dir_binsize,
        save_intermediate_data = save_intermediate_data_tmp,
        swap_AB = swap_AB,
  ...
  if (sub_domains == TRUE)
  {
    silence_out = foreach::foreach(chr = chrs) %do% ## use one core instead of multi-cores
      {
        ...
        intermediate_data_file = sprintf(
          '%s/%skb/chr%s_intermediate_data.Rds',
        ...
        try(CALDER_sub_domains(
          intermediate_data_file = intermediate_data_file,
          chr = chr,
          save_dir = save_dir_sub_domains,
          bin_size = bin_size
        ))
      }
    ...
    opt_sub_domain_beds = sprintf(
      '%s/chr%s_nested_boundaries.bed',
      file.path(save_dir, 'intermediate_data/sub_domains') ,
      gsub('chr', '', chrs, ignore.case = TRUE)
    )
    cmd_opt = sprintf("cat %s > %s",
                      paste0(opt_sub_domain_beds, collapse = " "),
                      save_opt_file)
    system(cmd_opt)
  }
}
