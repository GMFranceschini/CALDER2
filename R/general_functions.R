get_ccA_atanh = function(contact_mat_file,
                         compress_size,
                         bin_size)
{
  ...
  if (!all(contact_mat[[2]] >= contact_mat[[1]]))
  ...
  contact_mat_sparse[cbind(contact_mat[[1]] + 1, contact_mat[[2]] +
                             1)] <- contact_mat[[3]]
  
  rownames(contact_mat_sparse) = colnames(contact_mat_sparse) = as.character(1:nrow(contact_mat_sparse))
  
  contact_mat_sparse = Matrix::forceSymmetric(contact_mat_sparse, uplo =
                                                'U')
  # if(bin_size!=bin_size_initial) contact_mat_sparse = mat_10to40kb( contact_mat_sparse, bin_size, bin_size_initial )
  A = remove_blank_cols(contact_mat_sparse, sparse = TRUE, ratio =
                          zero_ratio) ## has the same rows/cols as A
  if (nrow(A) < 100)
    A = remove_blank_cols(contact_mat_sparse, sparse = TRUE, ratio = 0) ## when all are dense
  while (min(apply(A, 1, sd)) == 0)
    ## sometimes after removing the cols / rows, the remained part will all be 0
  {
    A = remove_blank_cols(A, sparse = TRUE, ratio = 1E-7) ## has the same rows/cols as A
    if (nrow(A) < 1)
      stop('ERROR IN GENERATING MEANINGFUL A at the data generating step')
  }
  ...
  A_compressed = compress_mat_fast(as.matrix(A_2_compress), compress_size =
                                     compress_size)
...
get_detailed_full_tree <- function(res_info)
{
  ...
  for (i in 1:nrow(res_info$segmentss))
  {
    ...
    igraph::V(res_info$hi_tree)$name = paste(
      '(',
      igraph::V(res_info$hi_tree)$left_rescaled,
      ',',
      igraph::V(res_info$hi_tree)$right_rescaled,
      ')',
      sep = ''
    )
  }
  ...
  for (i in 1:length(branches))
    branches[[i]] = update_branch_name(branches[[i]], root_start = res_info$segmentss[i, 1])
  ...
  igraph::V(full_tree)$right = substring(names_tmp[, 2], 1, nchar(names_tmp[, 2]) -
                                           1)
  if (!is_binary_tree(full_tree))
    stop("Trunk + branches do not produce a binary tree")
...
insulation_score_fun = function(A, size = 3)
{
  ...
  insulations = sapply(bin_starts, function(bin_start)
    insulation_score_fun_helper(bin_start))
...
generate_compartment_segs <- function(initial_clusters)
{
  ...
  if (!all(diff(bins_seq) == 1))
    stop('check generate_compartment_segs in generate_compartment_segs.R')
  if (bins_seq[1] != 1)
    stop('check generate_compartment_segs in generate_compartment_segs.R')
  compartment_segs = do.call(rbind, lapply(initial_clusters, function(v)
    v[c(1, length(v))]))
  compartment_segs = data.frame(start_pos = compartment_segs[, 1], end_pos =
                                  compartment_segs[, 2])
...
get_least_residue_matrix <-
  function(pA_sym, max_nbins, allowed_shifts)
  {
    nbins = nrow(pA_sym)
    ...
    if (max_nbins != max_nbins_bp)
      warning(
        'Value of max_nbins has been adjusted from ',
        max_nbins_bp,
        ' to ',
        max_nbins,
        '. This results in ',
        remainder,
        ' rows/columns excluded for the analysis.'
      )
    
    if (remainder != 0)
      A_final = pA_sym[-(tail(1:nbins, remainder)), -(tail(1:nbins, remainder))]
    if (remainder == 0)
      A_final = pA_sym
    res = list(A_final = A_final,
               n2one = n2one,
               max_nbins_new = max_nbins)
  }
...
{
  if (method == 'mean')
    return(mean(corr_mat[mod1, mod2]))
  if (method == 'median')
    return(median(corr_mat[mod1, mod2]))
  if (method == 'max')
    return(max(corr_mat[mod1, mod2]))
...
{
  zero_indices = apply(A, 2, function(v)
    all(v == 0))
  if (all(zero_indices == 0))
    return(A)
...
my_dist <-
  function(m) {
    ...
    sq <-
      rowSums(m * m)
    res = suppressWarnings(sqrt(outer(sq, sq, "+") - 2 * mtm))
    res[is.nan(res)] = 0
    diag(res) = 0
    return(res)
  }
...
MoC = function(P, Q)
{
  len_p = length(P)
  len_q = length(Q)
  if ((len_p == len_q) & (len_p == 1))
    return(1)
  grids = expand.grid(1:len_p, 1:len_q)
  vecs = apply(grids, 1, function(x) {
    u = x[1]
    v = x[2]
    length(intersect(P[[u]], Q[[v]])) ^ 2 / length(P[[u]]) / length(Q[[v]])
  })
...
correct_A_fast_divide_by_mean <-
  function(A,
           remove_zero = TRUE,
           divide_or_substract = 'divide',
           mean_or_median = 'mean')
  {
    f = function(i, d)
      d * (d - 1) / 2 + 1 + (2 * d + i) * (i - 1) / 2
    ...
    for (d in seq_along(orders))
      orders[[d]] = f(i = seq(1, n - d + 1), d = d)
    for (d in seq_along(orders))
      diag_values[[d]] = upper_tri_values[f(i = seq(1, n - d + 1), d = d)]
    
    if (mean_or_median == 'mean') {
      if (remove_zero)
        means = unlist(lapply(diag_values, function(v) {
          m = mean(v[v != 0])
          v = v * 0 + m
          v[is.na(v)] = 0
          return(v)
    ...
    if (divide_or_substract == 'divide')
      A_corrected = A / means_mat
    if (divide_or_substract == 'substract')
      A_corrected = A - means_mat
...
creat_phylo_object <- function(tree)
{
  creat_phylo_object_inner <- function(tree)
  {
    ...
    if (left_branch_flag)
      left_branch_newick = igraph::V(left_branch)$name
    if (!left_branch_flag)
      left_branch_newick = creat_phylo_object_inner(left_branch)
    
    if (right_branch_flag)
      right_branch_newick = igraph::V(right_branch)$name
    if (!right_branch_flag)
      right_branch_newick = creat_phylo_object_inner(right_branch)
    
    if (igraph::diameters[1] > igraph::diameters[2])
      tree_newick = paste(
        '(',
        left_branch_newick,
        ':',
...
divide_into_groups <- function(A, n_group)
{
  N = nrow(A)
  dists = 1:(N - 1)
  counts = sapply(dists, function(v)
    n_cells2compute(A, v))
...
get_leaves <- function(tree, type = 'name')
{
  ...
  if (type == 'name')
    return(igraph::V(tree)[leaf_indices]$name)
  if (type == 'igraph_node')
    return(igraph::V(tree)[leaf_indices])
  if (type == 'igraph')
    return(igraph::V(tree)[leaf_indices])
  if (type == 'index')
    return(leaf_indices)
...
get_segments <-
  function(hi_tree,
           binsize_thresh,
           return_segmentss_tree = FALSE)
  {
    leaf_widths =  get_leaves(hi_tree, type = 'igraph')$width_rescaled
    if (binsize_thresh <= min(leaf_widths))
    {
      warning(
        'Your input binsize_thresh has a value: ',
        binsize_thresh,
        ', which is smaller than the minimum of the leaf width of hi_tree: ',
        min(leaf_widths)
      )
    }
    
    nodes2rm = numeric()
    for (i in 2:igraph::vcount(hi_tree))
      ## root node not take into account
    {
      ...
      if (width <= binsize_thresh)
      {
        ...
        if (width_parent <= binsize_thresh)
          nodes2rm = c(nodes2rm, i)
      }
    }
...
get_tree_decoration = function(single_res_info,
                               decoration = TRUE,
                               distr,
                               n_parameters,
                               imputation_num = 1E2)
{
  ...
  # cat(dim(cA), '\n')
  if (!is.null(single_res_info$full_tree))
    tree = single_res_info$full_tree
  if (is.null(single_res_info$full_tree))
    tree = get_tree_v0(single_res_info)
  
  if (length(tree) == 1)
    if (class(tree) == "character" & tree == 'bad_tree')
      return(tree)
  ...
  for (i in setdiff(1:igraph::vcount(tree), leaf_indices))
  {
    ...
    mid = sort(c(
      as.numeric(twins[1]$left),
      as.numeric(twins[1]$right),
      as.numeric(twins[2]$left),
      as.numeric(twins[2]$right)
    ))[2]
    ...
    if (distr == 'lnorm')
    {
      ...
      {
        test_info_imp  = p_likelihood_ratio_lnorm(
          cA,
          head = as.numeric(node$left),
          mid = mid,
          tail = as.numeric(node$right),
      ...
      test_info_nimp = p_likelihood_ratio_lnorm(
        cA,
        head = as.numeric(node$left),
        mid = mid,
        tail = as.numeric(node$right),
    ...
    test_info_mean_diff = lognormal_mean_test(
      cA,
      head = as.numeric(node$left),
      mid = mid,
      tail = as.numeric(node$right)
  ...
  if (!is.null(imputation_num))
    igraph::V(tree)[leaf_indices]$imp_p = 1
...
add_boundary_binsignal_to_decrated_branches <-
  function(decorated_branches, bin_signals_5_10)
  {
    tree_widths = sapply(decorated_branches, function(v)
      igraph::V(v)[1]$width)
    if (sum(tree_widths) != length(bin_signals_5_10))
      stop('Check add_boundary_binsignal_to_decrated_branches')
    for (k in 1:length(decorated_branches))
    {
      ...
      for (i in setdiff(1:igraph::vcount(tree), leaf_indices))
      {
        ...
        # the p-value of: H0: unioned model; H1: hierarchical model
        twins = igraph::ego(tree, 1, node, mode = 'out', mindist =
                              1)[[1]]
        mid = sort(c(
          as.numeric(twins[1]$left),
          as.numeric(twins[1]$right),
...
get_tree_v0 <- function(single_res_info)
{
  ...
  recursive <- function(current_node)
  {
    ...
    if (is.na(flag_left))
      return('bad_tree') ## added: 30-04-2020
    
    left_leaf = paste(seqs[1], seqs[2], sep = '-')
    right_leaf = paste(seqs[3], seqs[4], sep = '-')
    
    if (flag_left)
      left_tree = recursive(left_node)
    if (!flag_left)
      left_tree = left_leaf
    
    if (flag_right)
      right_tree = recursive(right_node)
    if (!flag_right)
      right_tree = right_leaf
    ...
    tree = igraph::add_edges(tree_raw,
                             c(current_node, left_node, current_node, right_node))
  ...
  igraph::V(tree)$left = sapply(igraph::V(tree)$name, function(v) {
    tmp = strsplit(v, '-')[[1]]
    return(as.numeric(head(tmp, 1)))
  })
  igraph::V(tree)$right = sapply(igraph::V(tree)$name, function(v) {
    tmp = strsplit(v, '-')[[1]]
    return(as.numeric(tail(tmp, 1)))
  })
  igraph::V(tree)$name = sapply(igraph::V(tree)$name, function(v) {
    tmp = strsplit(v, '-')[[1]]
    paste('(', head(tmp, 1), ',', tail(tmp, 1), ')', sep = '')
  })
...
join_left_or_right <- function(tree)
{
  ...
  for (i in 2:(length(leaves) - 1))
  {
    ...
    if (parent$left < leave$left)
      left_or_right[i] = -1 ## left
    if (parent$right > leave$right)
      left_or_right[i] = 1 ## right
  }
...
long_slices <- function(left_or_right, thresh = 3)
{
  ...
  indices_of_slice = as.vector(unlist(mapply(
    function(u, v) {
      seq(from = u,
          length = v,
          by = 1)
    }, long_dup_start_pos, long_dup_lens
  )))
  dup_info = list(
    long_slices_lens = long_dup_lens,
    long_slices_start_pos = long_dup_start_pos,
    long_dup_direction = long_dup_direction,
    indices_of_slice = indices_of_slice
  )
  return(dup_info)
}
...
n_cells2compute <- function(A, dist, min_n_bins = 1)
{
  ...
  count = (dist - 2 * min_n_bins + 2) * (dist ^ 2 + 2 * dist * min_n_bins + dist - 2 *
                                           (min_n_bins - 2) * min_n_bins) * (N - dist) / 6
  return(count)
}
...
remove_blank_cols <-
           ...
           ratio = 0.05)
  {
    if (sparse == TRUE)
    {
      ...
      if (ratio > 0)
        ## remove col/rows with non-zero number smaller than 5% percentile
      {
...
tree_germination <- function(tree)
{
  ...
  {
    cat(leaf, '\n')
    if ((igraph::vcount(tree) - ecount(tree)) != 1)
      break
    bins = as.character(igraph::V(tree)[leaf]$left:igraph::V(tree)[leaf]$right)
    if (length(bins) == 1)
      stop('Some leave node contains only one bin, please check')
    if (length(bins) == 2)
      ## this is an exception. added on 11/06/2018
    {
      nodes_supp = as.character(bins)
      edges2add = c(leaf, bins[1], leaf, bins[2])
      tree = tree %>% igraph::add_vertices(length(nodes_supp), name =
                                             nodes_supp) %>% igraph::add_edges(edges2add)
      next
    }
...
trim_tree_adaptive <-
  function(tree,
           ...
           max_nimp_p = Inf,
           width_thresh = -Inf,
           boundary_signal_thresh = Inf,
           peak_thresh = -Inf)
  {
    if (igraph::vcount(tree) == 1)
      return(tree) ## so this can be used!
    ...
    nodes2rm = unique(
      c(
        ...
        nodes2rm_imp_p,
        nodes2rm_nimp_p,
        nodes2rm_boundary_signal,
        nodes2rm_not_peak
      )
    )
    nodes_not_2rm = setdiff(igraph::V(tree)$name, nodes2rm)
    
    flags = sapply(nodes2rm, function(v) {
      children = names(unlist(
        igraph::ego(
          tree,
          nodes = v,
          order = igraph::diameter(tree),
          mindist = 1,
          mode = 'out'
        )
      ))
      all(children %in% nodes2rm)
    })
    nodes2rm_final = names(unlist(
      igraph::ego(
        tree,
        nodes = nodes2rm[which(flags == TRUE)],
        order = igraph::diameter(tree),
        mindist = 1,
        mode = 'out'
      )
    ...
    if (!is.null(nodes2rm_final))
      tree = tree - nodes2rm_final
    if (is.null(nodes2rm_final))
      cat('No nodes are removed at the given p_thresh\n')
...
trim_tree_adaptive_width_thresh <-
  function(tree, width_thresh)
  {
    ...
    parentOfnodes2rm_width = names(unlist(
      igraph::ego(
        tree,
        node = nodes2rm_width,
        order = 1,
    ...
    index2rm = c(which(duplicated(parentOfnodes2rm_width)), which(duplicated(fromLast =
                                                                               TRUE, parentOfnodes2rm_width)))
    nodes2rm = nodes2rm_width[index2rm]
    tree = tree - nodes2rm
    if (!is_binary_tree(tree))
      stop('Error in trim_tree_adaptive_width_thresh')
    ...
    while (length(nodes2rm_width) > 0)
    {
      ...
      left_siblings2change_right = left_siblings[left_siblings$right == (igraph::V(tree)[node]$left -
                                                                           1)]
      right_siblings2change_left = right_siblings[right_siblings$left == (igraph::V(tree)[node]$right +
                                                                            1)]
...
trim_tree_adaptive_binsig_thresh <-
  function(tree, boundary_signal_thresh)
  {
    ## merge TADs that are too small
    parent_of_nodes2rm = igraph::V(tree)[which(igraph::V(tree)$binsignal <= boundary_signal_thresh)]$name
    leaves = get_leaves(tree)
    nodes2rm = intersect(leaves,  names(unlist(
      igraph::ego(
        tree,
        nodes = parent_of_nodes2rm,
        order = 1,
        mindist = 1,
        mode = 'out'
      )
    ...
    while (length(nodes2rm) > 0)
    {
      ...
      if (length(twins_of_sibling) > 0)
        tree = igraph::add_edges(tree,
                                 c(
                                   parent$name,
                                   twins_of_sibling[1],
                                   parent$name,
...
visualize_left_or_right <- function(left_or_right, ...)
{
  ...
  for (i in 1:len)
  {
    if (left_or_right[i] == 1)
      segments(i - 0.1, 1, i + 1, 1, col = 'red', ...)
    if (left_or_right[i] == -1)
      segments(i - 0.1, -1, i + 1, -1, col = 'blue', ...)
  }
}

xenocraft <- function(trunk, branches)
{
  xenocraft_nodes = sapply(branches, function(v)
    igraph::V(v)[1]$name)
  if (!all(xenocraft_nodes %in% igraph::V(trunk)$name))
    stop("Check xenocraft function")
  
  children_xenocraft_nodes = unique(unlist(sapply(igraph::ego(
    trunk,
    order = igraph::diameter(trunk),
    node = xenocraft_nodes,
    mode = 'out',
    mindist = 1
  ), function(v)
    v$name)))
  ...
  for (att in att2delete)
    full_tree = delete_vertex_attr(full_tree, att)
  return(full_tree)
}
...
get_bin_singals_CHiP <-
  function(chr,
           ...
           ChiP_NAME,
           CELL_LINE,
           ChiP_data_already_loaded = FALSE)
  {
    #################
    ...
    ordered_bin_signals_ALL_k_list = lapply(ks, function(k) {
      hc_k_labels = get_cluser_levels(hc_ordered,
                                      k_clusters = k,
                                      balanced_4_clusters = FALSE)$cluster_labels
      
      if (is.null(ChiP_NAME))
      {
        CD_index = labels(hc_ordered)
        pos_start_end = pos_start_end[match(CD_index, rownames(pos_start_end)),]
        ordered_bin_signals_ALL = data.frame(
          CD_index = CD_index,
          n_bins = n_bins_of_CD[CD_index],
          pos_start = pos_start_end[, 1],
          pos_end = pos_start_end[, 2],
          compartment = hc_k_labels[CD_index]
        )
        return(ordered_bin_signals_ALL)
      }
      
      cluster_signal = get_names_by_H3k4me1(
        chr,
        res,
        ChiP_NAME,
        kmeans_cluster_assigment = hc_k_labels,
        CELL_LINE = CELL_LINE,
        ChiP_data_already_loaded = ChiP_data_already_loaded
      )
      ...
      ordered_bin_signals_ALL = data.frame(
        ...
        pos_end = pos_start_end[, 2],
        compartment = hc_k_labels[CD_index],
        ordered_bin_signals_ALL
      )
      return(ordered_bin_signals_ALL)
    })
    names(ordered_bin_signals_ALL_k_list) = paste0('clusters_', ks)
    names(ordered_bin_signals_ALL_k_list)[which(names(ordered_bin_signals_ALL_k_list) ==
                                                  "clusters_Inf")] = 'compartment_domains'
...
build_chr_bin_domain_fun <-
           ...
           compress_size = 10)
  {
    ...
    chr_bin_domain_tmp = lapply(chrs, function(chr)
      get_clusters_bins_xy(
        ...
        p_thresh,
        ob_oe,
        downsratio = downsratio,
        compress_size = compress_size
      ))
    names(chr_bin_domain_tmp) = chrs ## can use mapply
    chr_bin_domain = lapply(chrs, function(v)
    {
      chr_bin_domain_ind = data.frame(
        chr = paste0('chr', v),
        bin_index = as.numeric(unlist(chr_bin_domain_tmp[[v]])),
        intra_domain = rep(
          names(chr_bin_domain_tmp[[v]]),
          sapply(chr_bin_domain_tmp[[v]], length)
        )
      )
...
build_chr_bin_domain_fun_direct <-
  function(chr,
           ...
           cluster_vec,
           bin_names)
    ## directly from R workingspace instead of loading
  {
    chrs = as.character(chr) ## for historic reason, chr -- chrs
    chr_bin_domain_tmp = lapply(chrs, function(chr)
      get_cluster_bin_names(initial_clusters, cluster_vec, bin_names))
    names(chr_bin_domain_tmp) = chrs ## can use mapply
    chr_bin_domain = lapply(chrs, function(v)
      data.frame(
        chr = paste0('chr', v),
...
get_clusters_bins_xy <-
           ...
           sort = TRUE)
  {
    ...
    if (!is.null(downsratio))
      compartments_Rdata_file = paste0(
        sub_folder,
        '/chr',
        chr,
        '_compartments_atanh_log_AB',
        ws,
        'downsratio_',
        downsratio,
        '.Rdata'
    ...
    if (is.null(downsratio))
      compartments_Rdata_file = paste0(sub_folder,
                                       '/chr',
                                       chr,
                                       '_compartments_atanh_log_AB3_3.Rdata')
...
get_cluster_bin_names <-
  ...
  {
    if (sort == TRUE)
      cluster_indices = sort(unique(cluster_vec), decreasing = TRUE)
    if (sort == FALSE)
...
reorder_dendro <-
           ...
           aggregateFun = mean)
  {
    get_children <- function(g, root_node)
    {
      ...
      children = intersect(leaves,
                           igraph::ego(
                             g,
                             mode = 'out',
                             root_node,
                             order = igraph::diameter(g)
                           )[[1]]$name)
      return(children)
    }
    ...
    igraph::V(g)$weight = sapply(1:igraph::vcount(g), function(v)
      aggregateFun(named_weights[get_children(g, v)]))
...
get_cluser_assignment = function(hc, k_clusters, leaves_hclust_pc)
{
  ...
  clusters = tapply(as.numeric(names(clusters_raw)), clusters_raw, function(v)
    list(v))
  
  od = rank(sapply(clusters, function(v)
    sort(match(
      v, as.numeric(leaves_hclust_pc)
    ))[1]))
  
  cluster_assignment = numeric(sum(sapply(clusters, length)))
  for (j in 1:length(clusters))
    cluster_assignment[clusters[[j]]] = od[j]
...
get_cluster_boudaries <- function(hc, k_clusters, named_weights)
{
  ...
  clusters = tapply(as.numeric(names(clusters_raw)), clusters_raw, function(v)
    list(v))
  clusters_pc_rank = lapply(clusters, function(v)
    unname(sort(rank(named_weights)[v])))
  bondaries = sapply(clusters_pc_rank, function(v)
    tail(v, 1))
...
get_cluser_levels <-
  function(hc_ordered,
           k_clusters,
           balanced_4_clusters = FALSE)
    ## get the detailed A1, A2, ..., B1, B2...
  {
    assign_twins_name <- function(graph, node)
    {
      twins = igraph::ego(graph,
                          node,
                          mode = 'out',
                          order = 1,
                          mindist = 1)[[1]]$name
      ...
      if (node == igraph::V(graph)[1]$name)
        igraph::V(graph)[twins]$level_name = c('B', 'A')
      return(graph)
    }
    
    if (k_clusters == Inf)
      k_clusters = length(labels(as.dendrogram(hc_ordered)))
    #################################################
    # cat('haha I am here\n')
    ...
    for (node in bfs_names)
    {
      ...
      if (!any(is.na(igraph::V(graph)$level_name)))
        break ## when all common_father_name have level_name
    }
    
    if (balanced_4_clusters == TRUE)
      ##A1, A2, B1, B2
    {
      ...
      children = igraph::ego(
        graph,
        order = igraph::diameter(graph),
        nodes = branch_root,
        mode = 'out',
        mindist = 0
      ) ## mindist==0, when itself is a branch
      tmp = lapply(children, function(v)
        intersect(leave_names, v$name))
    ...
    {
      if (length(u) == 1)
        return(which(u == igraph::V(graph)$name))
      return(max(which(sapply(igraph::ego(
        graph,
        order = igraph::diameter(graph),
    ...
    if (!all(diff(match(common_father_name, dfs_names)) >= 0))
      stop('!!!!!!!!check get_all_children in header_funs.R')
    
    
    
    level_names = igraph::V(graph)[common_father_name]$level_name ## label names are ordered
    if (any(sort(level_names, decreasing = TRUE) != level_names))
      warning('!!!!!!!!Need to check get_all_children in header_funs.R')
...
get_original_tad_indices <-
  function(names_A_final, TADs, bin_size)
  {
...
update_branch_name <- function(branch, root_start)
{
  ...
  igraph::V(branch)$name = paste('(',
                                 igraph::V(branch)$left,
                                 ',',
                                 igraph::V(branch)$right,
                                 ')',
                                 sep = '')
  return(branch)
}
