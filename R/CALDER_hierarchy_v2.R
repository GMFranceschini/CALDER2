...
make_comp_tab_calder <- function(comp_raw, bin_size) {
  ...
  comp_tab_ALL <- foreach::foreach(
    comp_raw_chr = split(comp_raw,
      by =
        "chr"
    ),
    .combine = rbind
...
generate_domain_2D_bedpe <- function(sub_compartment_file, bin_size, bedpe_file, n_sub = 8) {
  ...
  comp_raw_li <- split(data.table::fread(sub_compartment_file, skip = my_skip),
    by =
      "chr"
  )
...
get_cor_with_ref <- function(chr_bin_pc, bin_size, ref_compartment_file = NULL, feature_track = NULL) {
  ...
  get_binned_vals <- function(feature_track_chr, bin_size = 5E3) {
    ...
    GR <- GenomicRanges::makeGRangesFromDataFrame(feature_track_chr,
      keep.extra.columns =
        TRUE
    )
...
CALDER_CD_hierarchy_v2 <- function(contact_tab_dump = NULL,
  ...
  ## switch PC direction based on correlation with "ground_truth"
  {
    ...
    if (!is.null(ref_compartment_file)) {
      cor_with_ref <- try(get_cor_with_ref(chr_bin_pc, bin_size2look,
        ref_compartment_file =
          ref_compartment_file
      ))
    }
    ...
    } ## force swap PC direction if in some case the A/B direction is reverted
  ...
  hc <- res$CALDER_hc
  hc_k_labels_full <- try(get_cluser_levels(hc,
...
get_best_reorder <- function(hc_hybrid_x_pro, x_pro) {
  ...
  reordered_names_x_pro_list[[1]] <- reorder_dendro(hc_hybrid_x_pro, (x_pro),
    aggregateFun =
      mean
  ) ## here the clusters are assigned into A.1 A.2 B.1 B.2
...
CALDER_sub_domains <- function(intermediate_data_file = NULL, intermediate_data = NULL, chr, save_dir, bin_size) {
  ...
  }
  {
    ...
    if (!setequal(
      rownames(intermediate_data$mat),
