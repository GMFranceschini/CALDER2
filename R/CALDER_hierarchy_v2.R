...
make_comp_tab = function(comp_raw, bin_size){
  ...
  bin_indices_li = apply(comp_raw[, 2:3], 1, function(v) {
    bins = 1 + seq(v[1] - 1, v[2], by = bin_size) / bin_size
  ...
  comp_tab = data.table::data.table(
    chr = rep(comp_raw[[1]], expand_len),
    bin_index = unlist(bin_indices_li),
    comp_name = rep(comp_raw[[4]], expand_len),
    comp_rank = rep(comp_raw[[5]], expand_len)
  )
...
make_comp_tab_calder = function(comp_raw, bin_size)
{
  ...
  {
    bin_indices_li = apply(comp_raw_chr[, 2:3], 1, function(v) {
      bins = 1 + seq(v[1] - 1, v[2], by = bin_size) / bin_size
      bins[1:(length(bins) - 1)]
    })
    expand_len = sapply(bin_indices_li, length)
    ...
    comp_tab = data.table::data.table(
      chr = rep(comp_raw_chr[[1]], expand_len),
      bin_index = unlist(bin_indices_li),
      comp_name = rep(comp_raw_chr[[4]], expand_len),
      comp_rank = rep(comp_raw_chr[[5]], expand_len)
    )
    comp_tab$continous_rank = unname(continous_rank[comp_tab$comp_name])
  ...
  comp_tab_ALL = foreach::foreach(comp_raw_chr = split(comp_raw, by =
                                                         "chr"),
                                  .combine = rbind) %do%
    {
...
generate_domain_2D_bedpe = function(sub_compartment_file,
                                    bin_size,
                                    bedpe_file,
                                    n_sub = 8)
{
  generate_domain_2D_bedpe_chr = function(comp_raw)
  {
    ...
    n_row = nrow(comp_tab)
    comp_tab$is_boudary = c(1 * (comp_tab$comp_name[2:n_row] != comp_tab$comp_name[1:(n_row -
                                                                                        1)]), 0)
    
    pos_end = union(comp_tab[is_boudary == 1]$pos_end, comp_tab[n_row,]$pos_end)
    pos_start = union(comp_tab[1,]$pos_start, pos_end[1:(length(pos_end) -
                                                           1)] + 1)
  ...
  my_skip = as.numeric(strsplit(system(
    intern = TRUE, sprintf('zgrep -n "chr" %s', sub_compartment_file)
  ), ':')[[1]][1])
  comp_raw_li = split(data.table::fread(sub_compartment_file, skip = my_skip), by =
                        "chr")
  bedpe = do.call(rbind, lapply(comp_raw_li, generate_domain_2D_bedpe_chr))
  
  data.table::fwrite(bedpe,
                     file = bedpe_file,
                     col.names = FALSE,
                     sep = "\t")
  return(bedpe)
...
CALDER_CD_hierarchy_v2 = function(contact_tab_dump = NULL,
                                  contact_file_dump = NULL,
                                  contact_file_hic = NULL,
                                  chr,
                                  bin_size_input,
                                  bin_size2look,
                                  save_dir,
                                  ...
                                  feature_track = NULL)
{
  ...
  get_cor_with_ref = function(chr_bin_pc,
                              bin_size,
                              ref_compartment_file = NULL,
                              feature_track = NULL)
    ## correlation of PC1 with comp. domain rank of genome
  {
    ext_chr_bin_pc = function(chr_bin_pc)
      ## spand chr_bin_pc using 5kb bin
    {
      bin_indices_li = unlist(apply(chr_bin_pc[, 4:5], 1, function(v) {
        bins = 1 + seq(v[1] - 1, v[2], by = 5E3) / 5E3
        list(bins[1:(length(bins) - 1)])
      }), recursive = FALSE)
      expand_len = sapply(bin_indices_li, length)
      chr_bin_pc_ext = data.table::data.table(
        chr = rep(chr_bin_pc[[1]], expand_len),
        bin_index = unlist(bin_indices_li),
        PC1_val = rep(chr_bin_pc$PC1_val, expand_len)
      )
      
      chr_bin_pc_ext$pos_start = (chr_bin_pc_ext$bin_index - 1) *
        5E3 + 1
    ...
    get_binned_vals = function(feature_track_chr, bin_size = 5E3)
    {
      ...
      binnedMean <- function(bins, numvar, mcolname)
      {
        ...
        bins_per_chrom <-
          split(GenomicRanges::ranges(bins),
                GenomicRanges::seqnames(bins))
        ...
        new_mcol <-
          unsplit(sums_list, as.factor(GenomicRanges::seqnames(bins)))
        GenomicRanges::mcols(bins)[[mcolname]] <- new_mcol
        return(bins)
      }
      
      
      GR = GenomicRanges::makeGRangesFromDataFrame(feature_track_chr, keep.extra.columns =
                                                     TRUE)
      GR_chrs = split(GR, GenomicRanges::seqnames(GR))
      seq_lens = sapply(GR_chrs, function(v)
        max(GenomicRanges::end(v)))
      ...
      bins = GenomicRanges::tileGenome(seq_info,
                                       tilewidth = bin_size,
      ...
      bin_val = data.table::data.table(
        chr = bin_val_tmp$seqnames,
        bin_index = bin_val_tmp$end / bin_size,
        continous_rank = log2(1 + bin_val_tmp$bin_val - min(bin_val_tmp$bin_val))
      )
      return(bin_val)
    }
  ...
  cat(
    '>>>> Begin process contact matrix and compute correlation score at:',
  ...
  processed_data = contact_mat_processing_v2(
    contact_tab_dump,
    contact_file_dump = contact_file_dump,
    contact_file_hic = contact_file_hic,
    chr = chr_num,
    bin_size_input = bin_size_input,
    bin_size2look = bin_size2look,
  ...
  cat(
    '[',
    as.character(chr),
    '] Finish process contact matrix and compute correlation score at:',
    as.character(Sys.time()),
    '\n'
  )
  cat(
  ...
  cat(
    '>>>> Begin compute compartment domains and their hierachy at:',
    as.character(Sys.time()),
    '\n',
    file = log_file,
    append = TRUE
  ...
  compartments[[2]] = generate_compartments_bed(
    ...
    ccmat_dense_compressed_log_atanh,
    p_thresh,
    out_file_name = NULL,
  ...
  trend_mean_list = lapply(1:4, function(v)
    1 * (mat_dense_cluster_mean[, -(1:v)] > mat_dense_cluster_mean[, -n_clusters - 1 + (v:1)]))
  ...
  {
    initial_clusters_ori_bins = lapply(initial_clusters, function(v)
      as.numeric(bin_names[v]))
    chr_bin_pc = data.table::data.table(
      chr = chr_name,
      bin = unlist(initial_clusters_ori_bins),
      PC1_val = rep(PC_12_atanh[, 1], sapply(initial_clusters_ori_bins, length))
    )
    ...
    if (!is.null(ref_compartment_file))
      cor_with_ref = try(get_cor_with_ref(chr_bin_pc, bin_size2look, ref_compartment_file =
                                            ref_compartment_file))
    ## get correlation with supplied "ground truth or reference"
    if (is.null(ref_compartment_file))
      cor_with_ref = try(get_cor_with_ref(chr_bin_pc, bin_size2look, feature_track =
                                            feature_track))
    
    
    if (class(cor_with_ref) == 'try-error')
      cor_with_ref = 1 ## psudo cor
    if (!is.null(ref_compartment_file))
      cat(
        '[',
        as.character(chr),
        '] Correlation between PC1 and reference compartment is :',
        format(abs(cor_with_ref), digits = 5),
        '\n'
      )
    if (is.null(ref_compartment_file))
      cat(
        '[',
        as.character(chr),
        '] Correlation between PC1 and feature_track is :',
        format(abs(cor_with_ref), digits = 5),
        '\n'
      )
    ...
    if (swap_AB == 1)
      PC_direction = -PC_direction ## force swap PC direction if in some case the A/B direction is reverted
  ...
  hc = res$CALDER_hc
  hc_k_labels_full = try(get_cluser_levels(hc, k_clusters = Inf, balanced_4_clusters =
                                             FALSE)$cluster_labels)
  ...
  res$comp = bin_comp
  res$CDs = lapply(res$initial_clusters, function(v)
    res$bin_names[v])
  res$mat = mat_dense
  res$chr = chr
  ...
  cat(
    '>>>> Finish compute compartment domains and their hierachy at: ',
    as.character(Sys.time()),
    '\n',
    file = log_file,
    append = TRUE
  ...
  if (!is.null(ref_compartment_file))
    cat(
      'Correlation between PC1 and reference compartment domain rank on this chr is: ',
      format(abs(cor_with_ref), 1, 5),
      '\n',
      file = log_file,
      append = TRUE
    )
  if (is.null(ref_compartment_file))
    cat(
      'Correlation between PC1 and feature_track on this chr is: ',
      format(abs(cor_with_ref), 1, 5),
      '\n',
      file = log_file,
      append = TRUE
  ...
  cat(
    '\n\n',
    'Total time used for computing compartment domains and their hierachy:',
    timediff,
    '\n',
    file = log_file,
    append = TRUE
...
project_to_major_axis <- function(PC_12_atanh)
{
  ...
  more_x = seq(min(PC_12_atanh[, 1]),
               max(PC_12_atanh[, 1]),
               len = 10 * length(PC_12_atanh[, 1]))
  major_axis = cbind(x = more_x, y = predict(loess_fit, newdata = more_x))
  new_x_axis = cumsum(c(0, sqrt(
    diff(major_axis[, 1]) ^ 2 + diff(major_axis[, 2]) ^ 2
  ))) ## the new xaxis on the curved major_axis
...
get_best_reorder <- function(hc_hybrid_x_pro, x_pro)
{
  ...
  reordered_names_x_pro_list[[1]] = reorder_dendro(hc_hybrid_x_pro, (x_pro), aggregateFun =
                                                     mean) ## here the clusters are assigned into A.1 A.2 B.1 B.2
  
  best_index = which.max(sapply(reordered_names_x_pro_list, function(v)
    cor(1:n, unname(
      rank(x_pro, ties.method = 'first')[v]
    ))))
...
generate_hierachy_bed = function(chr, res, save_dir, bin_size)
{
  ...
  compartment_info_tab = create_compartment_bed_v4(chr_bin_domain, bin_size =
                                                     bin_size)
  ...
  options(scipen = 999)
  write.table(
    compartment_info_tab,
    file = compartments_tsv_file,
    quote = FALSE,
    sep = '\t',
    row.names = FALSE
  )
  ...
  compartment_bed = data.frame(
    chr = paste0('chr', compartment_info_tab$chr),
    compartment_info_tab[, 2:4],
    comp_val[comp_8],
    '.',
  ...
  bounday_bed = data.frame(
    chr = paste0('chr', compartment_info_tab$chr),
    compartment_info_tab[, 3],
    compartment_info_tab[, 3],
    '',
    '.',
    compartment_info_tab[, 3],
...
{
  {
    ...
    from_id = as.numeric(bins[c(1, head(borders, length(borders) - 1) +
                                  1)])
    ...
    compartment_info_tab = data.frame(
      chr = rep(unique(chr), length(pos_start)),
      pos_start = pos_start,
      pos_end = pos_end,
      domain = v$comp[borders]
    )
  }
                              ...
                              bin_size)
{
  ...
  if (is.null(intermediate_data))
    intermediate_data = readRDS(intermediate_data_file)
  {
    if (intermediate_data$chr != chr)
      stop('intermediate_data$chr!=chr; check your input parameters\n')
    if (!setequal(rownames(intermediate_data$mat),
                  intermediate_data$bin_names))
      stop('!setequal(rownames(intermediate_data$mat), intermediate_data$bin_names) \n')
    compartment_segs = generate_compartment_segs(intermediate_data$initial_clusters)
    
    cat(
      '[',
      as.character(chr),
      '] Begin compute sub-domains within each compartment domain at:',
      as.character(Sys.time()),
      '\n'
    )
    cat(
    ...
    sub_domains_raw = HRG_zigzag_compartment_domain_main_fun(intermediate_data$mat,
                                                             './',
                                                             compartment_segs,
                                                             min_n_bins = 2)
    
    no_output = post_process_sub_domains(
      chr,
      sub_domains_raw,
      ncores = 1,
      out_dir = save_dir,
    ...
    cat(
      '>>>> Finish compute sub-domains within each compartment domain at:',
      as.character(Sys.time()),
      '\n',
      file = log_file,
      append = TRUE
    ...
    cat(
      '\n\n',
      'Total time used for computing sub-domains:',
      timediff,
      '\n',
      file = log_file,
      append = TRUE
