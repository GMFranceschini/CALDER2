...
build_comp_table_opt = function(save_dir, chrs, bin_sizes, with_ref)
{
  ...
  
  {
        ...
        consist_tab = foreach::foreach(bin_size = bin_sizes, .combine = merge) %do%
          {
            ...
            save_dir_binsize = file.path(save_dir,
                                         ...
                                         bin_size_kb)
            consist_tab_tmp = data.table::data.table(chr = paste0('chr', chrs), val =
                                                       0)
    ...
    # silent_out = foreach::foreach(identity=identities, .combine=merge) %do% ## do not use dopar
    {
      ...
      cor_with_ref = foreach::foreach(j = 1:nrow(opt_bin_tab), .combine =
                                        c) %do%
        {
      ...
      cor_with_ref_tab = data.table::data.table(
        chr = consist_tab$chr,
        cor = cor_with_ref,
        chr_num = gsub("chr", "", opt_bin_tab$chr)
      )
      ...
      p = ggplot2::ggplot(data = cor_with_ref_tab, ggplot2::aes(
        x = index,
        y = cor,
        label = format(round(cor_with_ref_tab$cor, 2), nsmall = 2)
      )) + ggplot2::geom_line(color = "red") + ggplot2::geom_point()
      p = p + ggplot2::geom_label() + ggplot2::scale_x_continuous(labels =
                                                                    cor_with_ref_tab$chr_num,
                                                                  breaks = 1:nrow(cor_with_ref_tab))
      p = p + ggplot2::geom_hline(
        yintercept = ifelse(with_ref, 0.4, 0.15),
        linetype = 2,
        color = 'darkblue'
      )
      if (with_ref)
        p = p + ggplot2::xlab('chr') + ggplot2::ylab('Rho') + ggplot2::ggtitle(
          'Correlation of compartment rank with reference\nRho < 0.4 indicates imprecise compartment calling'
        )
      if (!with_ref)
        p = p + ggplot2::xlab('chr') + ggplot2::ylab('Rho') + ggplot2::ggtitle(
      ...
      opt_sub_comp_beds = foreach::foreach(i = 1:nrow(opt_bin_tab)) %do%
        {
          ...
          opt_sub_comp_bed_chr = sprintf(
            "%s/%s_sub_compartments.bed",
            file.path(
              save_dir,
              'intermediate_data/sub_compartments',
              opt_bin_size_kb
      ...
      cmd_opt = sprintf("cat %s > %s",
                        paste0(opt_sub_comp_beds, collapse = " "),
                        save_opt_file)
      system(cmd_opt)
